...
`bootstrap-dependencies.ps1` requires the receipt-backed unseeded `runtime\venv`, the pinned managed CPython and uv layers, and the exact committed dependency-lock digest. It never syncs in place: the current venv is moved to a managed backup, a fresh venv is created from the pinned base interpreter, and `uv pip sync` materializes the lock with required hashes, wheels only, `link-mode=copy`, contained uv cache/config state, and Python downloads disabled. The new environment must match the accepted dependency set exactly and pass `uv pip check` before the dependency receipt is committed. Any materialization or receipt-commit failure restores the previous venv.

A receipt-backed dependency-ready environment is idempotent without `-Force`; unexpected distributions or receipt drift fail closed. `-Force` performs a full transactional replacement rather than mutating the live environment. `-Offline` is available when every required artifact is already present in the contained uv cache.
