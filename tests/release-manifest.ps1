...
if(@($release.files).Count -ne 28){throw "Expected 28 owned distribution files, got $(@($release.files).Count)."}
$seen=@{}
...
}foreach($required in @('install.ps1','start.ps1','update.ps1','uninstall.ps1','scripts/common.ps1','scripts/lifecycle.ps1','scripts/update-planner.ps1','scripts/update-staging.ps1','scripts/update-transaction.ps1','scripts/env.ps1','config.example.psd1','LICENSE','THIRD_PARTY_NOTICES.md')){if(-not$seen.ContainsKey($required.ToLowerInvariant())){throw "Required distribution file missing from release manifest: $required"}}
